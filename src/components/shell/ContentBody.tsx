...
type Props = {
  /**
   * The content's header. Rendered *over* the scrolling box rather than inside
  ...
  className?: string;
  /** The editor uses this to flush a pending save when focus leaves the content. */
  onBlur?: (event: FocusEvent<HTMLDivElement>) => void;
};

/**
 * The scrolling box inside a `.content`, and the one bit of client state a content area
 * needs: whether anything has scrolled under the header.
 *
 * It scrolls inside the content, not as the content, so the wash stays put. The
 ...
 *
 * The `scrolled` flag goes on the frame and `.content-head` reads it from any
 * ancestor, so a server-rendered page (the gallery) gets a live glass header
 * by wrapping its contents.
 */
export function ContentBody({ head, children, className, onBlur }: Props) {
  const [scrolled, setScrolled] = useState(false);

  return (
    // The blur listener sits here rather than on the scroller so it still
    // means "focus left the content" now that the header's controls are outside
    // the scrolling box.
    <div onBlur={onBlur} data-scrolled={scrolled} className="relative h-full">
      ...
      <div
        ...
        // flicker in and out at the top of the content.
        onScroll={(event) => setScrolled(event.currentTarget.scrollTop > 2)}
        className={`content-scroller h-full overflow-y-auto${className ? ` ${className}` : ""}`}
      >
        {children}
      </div>
    </div>
